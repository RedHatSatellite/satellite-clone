...
1. You will need files from a katello-backup (`katello-backup` on the `Satellite server`).
   ...
   - RHEL 6 to 7 migration scenario  : config_files.tar.gz, mongo_data.tar.gz, foreman.dump, candlepin.dump, (optional) pulp_data.tar. (A helper script /usr/share/satellite-clone/helpers/rhel_6_to_7_migration_backup.sh is included for easily creating these files.)
